...
def weights(gdps, cbr_historic):
    countries = cbr_historic.sort_values('fecha', 
                                      ascending=False).drop_duplicates(subset=["pais"], 
                                                                       keep='last')
    ...
    for i in countries['pais']:
        for j in range(min_year, max_year + 1):
            weights = weights.append({'ano': j,
                                      'pais':i}, ignore_index=True)
    gdps = gdps.replace({"pais": contries_dict})
    weights = weights.merge(gdps, 
                            how='left', 
                            on=['pais','ano'])
    weights = weights.set_index('pais').groupby(level='pais').ffill().reset_index().dropna()
    weights_agg = weights.groupby(['ano','pais']).agg({'gdp': 'sum'})
    percents_df = weights_agg.groupby(level=0).apply(lambda x: x / float(x.sum()))
...
def get_cbr_data(days=180):
    ...
    series_index = series_nw.merge(series_w, 
                                   how='left',
                                   on=['fecha']).merge(
                                       tpm,
                                       how='left',
