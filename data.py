...
def get_gdps():
    ...
    df.rename(columns={df.columns[0]: "pais"}, inplace=True)
    valid_columns = []
    for i in df.columns:
        try:
            if i in ["pais"]:
    ...
    df = df[valid_columns]
    df = (
        pd.melt(df, id_vars="pais", value_vars=valid_columns.remove("pais"))
        .rename(columns={"variable": "ano", "value": "gdp"})
        .dropna()
    )
